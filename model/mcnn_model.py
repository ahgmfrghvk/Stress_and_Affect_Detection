...
class MRCNN(nn.Module):
    def __init__(self, afr_reduced_cnn_size):
        ...
        self.features1 = nn.Sequential(
            nn.Conv1d(1, 64, kernel_size=260, stride=30, bias=False, padding=130),
            nn.BatchNorm1d(64),
            #self.GELU,
            nn.ReLU(inplace=True),
            nn.Conv1d(64, 64, kernel_size=260, stride=30, bias=False, padding=130),
            nn.BatchNorm1d(64),
            #self.GELU,
            nn.ReLU(inplace=True),
            nn.AdaptiveMaxPool1d(50),
            #nn.Dropout(drate),
        )

        self.features2 = nn.Sequential(
            nn.Conv1d(448, 128, kernel_size=5, stride=2, bias=False, padding=2),
            nn.ReLU(inplace=True),
    ...
    def forward(self, x_o, x_ma4, x_ma8, x_ma16, x_ma32, x_dn3, x_dn2):
        x1 = self.features1(x_o)
        x2 = self.features1(x_ma4)
        x3 = self.features1(x_ma8)
        x4 = self.features1(x_ma16)
        x5 = self.features1(x_ma32)
        x6 = self.features1(x_dn3)
        x7 = self.features1(x_dn2)
        x_concat = torch.cat((x1, x2, x3, x4, x5, x6, x7), dim=1)
        xtotal = self.features2(x_concat)
        xtotal = self.dropout(xtotal)
        xtotal = self.AFR(xtotal)
...
class AttnSleep(nn.Module):
    def __init__(self):
        ...
        d_model =25  # set to be 100 for SHHS dataset
    ...
    def forward(self, x_o, x_ma4, x_ma8, x_ma16, x_ma32, x_dn3, x_dn2):
        x_feat = self.mrcnn(x_o, x_ma4, x_ma8, x_ma16, x_ma32, x_dn3, x_dn2)
